...
WSMETHOD POST NEW WSRECEIVE WSSERVICE WSTITSE2
	...
	// Dados do título
	cCondPag  	:= AllTrim(jJson:GetJsonObject("condpag"))
	cChavePix 	:= AllTrim(jJson:GetJsonObject("chavepix"))
	cTipo		:= AllTrim(jJson:GetJsonObject("tipo"))
	cIdLcx		:= AllTrim(jJson:GetJsonObject("idlocx"))
	cNum 		:= PadL(cIdLcx, TamSX3('E2_NUM')[01],"0")

	// Valida se Id Locx está preenchido - Id é utilizado para a geração do número do título a pagar, onde cada titulo vindo do LOCX deve ter um ID único.
	If Empty(cIdLcx)
		Self:SetStatus(400)
		jResponse["errorId"]  := "BFREST-006"
		jResponse["error"]    := "ID LOCX não informado"
		jResponse["solution"] := "Informe o ID LOCX para geração do número do título a pagar."
		Self:SetResponse(EncodeUTF8(jResponse:ToJSON()))
		Return .F.
	EndIf

	// Validação: Fornecedor ou CNPJ/CPF obrigatórios - caso não tenha sido incluido vai disparar o erro para correção do Json
	If Empty(cFornec) .And. Empty(cCgc)
		Self:SetStatus(400)
		jResponse["errorId"]  := "BFREST-008"
		jResponse["error"]    := "Fornecedor não informado"
		jResponse["solution"] := "Informe o código do fornecedor ou o CNPJ/CPF."
		Self:SetResponse(EncodeUTF8(jResponse:ToJSON()))
		Return .F.
	EndIf

	// Faz a validação do fornecedor na tabela SA2, pegando pelo indicie 1, caso não encontre pelo código/loja, tenta pelo CNPJ (índice 3)
	DbSelectArea("SA2")
	If !Empty(cFornec)
		...
		If !Empty(cLoja)
			...
			If !SA2->(DbSeek(xFilial("SA2") + ;
					AvKey(cFornec,"A2_COD") + ;
					AvKey(cLoja,"A2_LOJA")))
				Return .F.
			EndIf

		Else
			// Se NÃO veio loja, pega a primeira loja do fornecedor
			If !SA2->(DbSeek(xFilial("SA2") + AvKey(cFornec,"A2_COD")))
				Return .F.
			EndIf
		...
		// FALLBACK: CNPJ
	ElseIf !Empty(cCgcJson)

		SA2->(DbSetOrder(3)) // FILIAL + CNPJ

		If !SA2->(DbSeek(xFilial("SA2") + cCgcJson))
			Return .F.
		EndIf
	...
	If cCgcSA2 <> cCgcJson
		...
		jResponse["solution"]       := "O código/loja informado pertence a outro CNPJ."
	...
	aAdd(aDados, {"E2_EMISSAO",     jJson:GetJsonObject("emissao"),  			Nil})
	...
	aAdd(aDados, {"E2_FORMPAG",     cCondPag,                       			Nil})
	...
	aAdd(aDados, {"E2_XIDLCX",      jJson:GetJsonObject("idlocx"),  			Nil})
	aAdd(aDados, {"E2_XCPIX",      	cChavePix, 									Nil})
...
/*/-----------------------------------------------------------------------------------
	...
	- version 1.1
-----------------------------------------------------------------------------------/*/
Static Function LimpaDoc(cCgc)

	Local cRet := AllTrim(cCgc)
